...
- `check-scripts-asmdef-references.ps1`: `-ProjectPath`, `-ScriptsRoots` (default `Assets/Scripts` and root `Packages`), plus **automatic** inclusion of each embedded **`Assets/Packages/com.scaffold.*`** directory when present (first-party only; other folders under `Assets/Packages` are not scanned), and each **`Library/PackageCache/com.scaffold.*`** directory when Unity has resolved git UPM packages, `-ExcludedAssemblyNames`, `-ExcludedGuidReferences`
