...
2. Assign on the bootstrap component: **Navigation Settings**, **navigation view holder**, **Addressable catalog publishers** (rebaked `AddressableScriptableObjectPublisherSO` assets; defaults under `Assets/GearEngine/Data/Bootstrap/`), **Race Session Defaults** (`RaceSessionDefaultsSO`, base roguelike car stats template), **BoardRulesSO** (grid size + motor cell authoring), **GearEngineFeatureToggleSO** (required), and **SplineCarRunnerConfigSO**. Catalogs load via Addressables addresses (`Catalogs/Track`, `Catalogs/Gear`, `Catalogs/RoguelikeGearPool`; see [`AddressableCatalogAddresses`](../../Assets/GearEngine/Scripts/App/Bootstrap/AddressableCatalogAddresses.cs)) from **`FoundationLayer`**, then ctor-inject into consumers in `CampaignLayer`.
...
Campaign catalog samples: `Assets/GearEngine/Data/Campaign/Catalogs/CampaignTrackCatalog.asset`, `CampaignGearCatalog.asset`, `CampaignRaceSessionDefaults.asset`, `CampaignRoguelikeGearPool.asset`.
