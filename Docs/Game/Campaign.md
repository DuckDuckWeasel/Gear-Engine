...
- **LiveOps client installers** — `CampaignTracksInstaller`, `CampaignGearCatalogInstaller`, `CampaignLoadoutInstaller`, `CampaignInventoryInstaller`, plus `CardsClientInstaller` and **`CampaignRoguelikeInstaller`**, run inside **`LiveOpsLayer`** so `IAsyncInitializable` hydration shares the same stack as `LiveOpsService`. Roguelike flow: [Roguelike.md](Roguelike.md), backend: [Roguelike module](../LiveOps/Roguelike.md).
