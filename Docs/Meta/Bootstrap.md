...
3. **[`LiveOpsLayer`](../../Assets/GearEngine/Scripts/App/Bootstrap/Layers/LiveOpsLayer.cs)** — `CloudCodeInstaller` + `LiveOpsInstaller` + `CurrencyClientInstaller`, then **game client installers** (constructor args). Meta uses the same **`CampaignTracksInstaller` / `CampaignGearCatalogInstaller` / `CampaignLoadoutInstaller` / `CampaignInventoryInstaller` / `CardsClientInstaller`** types as the Main scene, with **empty** `TrackCatalogSO` / `GearCatalogSO` instances created at runtime so smoke tests resolve the same module types without scene track data. [`LiveOpsService`](../../Assets/Packages/com.scaffold.liveops/Runtime/LiveOpsService.cs) implements `IAsyncInitializable` and runs the initial `GameDataRequest`.
