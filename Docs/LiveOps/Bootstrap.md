...
- 2026-04-19: Added Tracks, Loadout, Inventory, and Cards backend modules (`GameModule` + `IGameApiHandler`). **Client:** `CurrencyClientModule` + `CardsClientModule` install from `LiveOpsLayer`; `TracksClientModule`, `LoadoutClientModule`, and `InventoryClientModule` register from `CampaignScope` (scene `TrackCatalogSO` / `GearCatalogSO` coupling). Remote Config `Track.rc` + `Card.rc`. See [Tracks.md](Tracks.md), [Loadout.md](Loadout.md), [Inventory.md](Inventory.md), [Cards.md](Cards.md).
