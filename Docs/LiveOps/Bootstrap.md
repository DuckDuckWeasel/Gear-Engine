...
- 2026-04-19: Added Tracks, Loadout, Inventory, and Cards backend modules (`GameModule` + `IGameApiHandler`). **Client:** `CurrencyClientModule` installs from `CampaignLayer`; `TracksClientModule`, `LoadoutClientModule`, `InventoryClientModule`, and `CardsClientModule` register via **`CampaignLayer` installers** (`CampaignTracksInstaller`, `CampaignLoadoutInstaller`, `CampaignInventoryInstaller`, `CardsClientInstaller`); catalogs are Addressables-backed via publishers (see 2026-04-24). Remote Config `Track.rc` + `Card.rc`. See [Tracks.md](Tracks.md), [Loadout.md](Loadout.md), [Inventory.md](Inventory.md), [Cards.md](Cards.md).
- 2026-04-20: Added **Roguelike** module (perk roll + claim): DTO + `RoguelikeModule` + `DrawRoguelikeRollHandler` / `ClaimRoguelikePickHandler`; client `RoguelikeClientModule` + `CampaignRoguelikeInstaller`; `IRoguelikeRollService` in `CampaignLayer`; Remote Config [Roguelike.rc](../../Assets/LiveOps/RemoteConfig/Roguelike.rc). See [Roguelike.md](Roguelike.md).
- 2026-04-24: **Four-layer AppFlow stack** — `FoundationLayer` (sync, takes `NavigationSettings`+holder via ctor) + **`UgsLayer`** (`UgsInstaller` only — must be its own layer because `CloudCodeInstaller.Install` synchronously dereferences `CloudCodeService.Instance`, which is `null` until UGS finishes initializing) + **`LiveOpsLayer`** (Cloud Code, LiveOps, plus catalog publishers `TrackCatalogPublisher`/`GearCatalogPublisher`/`RoguelikeGearPoolPublisher`) + **`CampaignLayer`** (gameplay configs via ctor + LiveOps client installers + gear/sim/race services). Layers no longer override `PrepareAsync`; assets reach descendants exclusively through `AssetPublisherBase<T>` → `ILayerPublisher` (publish-to-descendants). `SequentialInLayerScheduler` is retained for in-layer `IAsyncInitializable` ordering. Addressables only carries `Catalogs/Track`, `Catalogs/Gear`, `Catalogs/RoguelikeGearPool`. Removed the split `LiveOpsServiceLayer`/`LiveOpsClientModulesLayer` and `CampaignGearCatalogInstaller`.
- 2026-04-25: **Catalog publishers unified in Foundation** — removed typed `TrackCatalogPublisher` / `GearCatalogPublisher` / `RoguelikeGearPoolPublisher` and the abstract `AddressableScriptableObjectPublisher<T>`. [`FoundationLayer`](../../Assets/GearEngine/Scripts/App/Bootstrap/Layers/FoundationLayer.cs) registers rebaked [`AddressableScriptableObjectPublisherSO`](../../Assets/GearEngine/Scripts/App/Bootstrap/Publishers/DataDriven/AddressableScriptableObjectPublisherSO.cs) entries from the bootstrap; [`LiveOpsLayer`](../../Assets/GearEngine/Scripts/App/Bootstrap/Layers/LiveOpsLayer.cs) is Cloud Code + LiveOps only. Default assets: [`Assets/GearEngine/Data/Bootstrap/`](../../Assets/GearEngine/Data/Bootstrap/). Address strings: [`AddressableCatalogAddresses`](../../Assets/GearEngine/Scripts/App/Bootstrap/AddressableCatalogAddresses.cs).
