...
- The Tutorial package provides the intended ScriptableObject-reference pattern (`TutorialVariableReference<TValue, TAsset>`), but it does not include a typed Flowchart-variable option.
- Existing Scaffold `*Data` structures already provide the Flowchart-variable and direct-value paths. Extending them is the lowest-risk way to update the majority of actions without altering their execution logic.
- The original `VariableDataDrawer<T>` accepted typed scene references correctly. The unified drawer keeps that behavior rather than relying on `FlowchartWindow`, which would make non-Flowchart inspectors and scene/global variable references unavailable.
- This repository has unrelated local changes. Only files necessary for this feature will be modified.

## Decision Log

- Use the Tutorial package's ScriptableObject-value pattern as the source-model reference, combined with the existing typed Flowchart `VariableBase<T>` model.
...
- Retain output-only action fields as writable Flowchart variables; they are output sinks rather than value inputs.
...
1. **Flowchart variable**: a typed `VariableBase<T>` reference, including global-scoped variables and compatible scene MonoBehaviours.
...
Then migrate all legacy raw action input references to existing or newly added typed `*Data` fields. Keep behavior and summary strings unchanged except for resolving through `.Value`. Do not migrate fields that are output targets: those require mutable Flowchart variables.
...
3. Move the generic `VariableDataDrawer<T>` into its own file and give it three explicit sources without using the active Flowchart window.
...
- A representative action such as `CrossfadeAnim` can select an Animator from a Flowchart variable, directly from the scene, or from an `Animator` value ScriptableObject.
...
- Regression tests cover Flowchart, direct, and ScriptableObject sources.
...
- `VariableBase<T>` supplies typed Flowchart values.
