namespace MaskTransitions
{
    ...
    public class TransitionManager : MonoBehaviour
    {
        ...
        #region Transition Between Canvases (same scene)
        /// <summary>
        /// Troca dois Canvas dentro da mesma cena, cobrindo a tela antes de trocar
        /// e revelando depois. Chame via CanvasSwitchTrigger para uso pelo Inspector.
        /// </summary>
        public void SwitchCanvas(GameObject fromCanvas, GameObject toCanvas, float delay = 0f)
        {
            StartCoroutine(SwitchCanvasWithWait(fromCanvas, toCanvas, delay));
        }

        IEnumerator SwitchCanvasWithWait(GameObject fromCanvas, GameObject toCanvas, float delay)
        {
            yield return new WaitForSeconds(delay);

            // Fecha a tela (mask cresce cobrindo tudo)
            Tween animationTween = StartAnimationForLoad();
            yield return animationTween.WaitForCompletion();

            // Troca os canvas enquanto a tela está coberta
            if (fromCanvas != null) fromCanvas.SetActive(false);
            if (toCanvas != null) toCanvas.SetActive(true);

            // Abre a tela de novo (revela o novo canvas)
            EndAnimation();
        }
        #endregion
    }
