...
{
    /// <summary>
    /// The meat of the Scaffold Collection. Internally uses a List of given type, simplest example
    /// being IntCollection. Provides some type specific additions to the base Collection for
    /// increasing speed and safety.
    ///
    /// Uses Promote methods to convert from objects or other collection or Scaffold.Variable types
    /// being passed in, will attempt to do compatability for you, such as if you give an
    /// IntCollection an int or a Scaffold.IntVariable, either works as the Promote is aware
    /// of Scaffold.VariableBase<T>. Will also allow mixing some operations between
    ...
    public class GenericCollection<T> : Collection
    {
        ...
        public override bool IsCollectionCompatible(object o)
        {
            ...
            {
                return ote is T || ote is Scaffold.VariableBase<T>;
            }
            else if (o is System.Collections.IList && otgs.Length > 0)
            {
                return otgs[0] == typeof(T) || otgs[0].IsSubclassOf(typeof(Scaffold.VariableBase<T>));
            }
            else
            {
                return false;
        ...
        protected virtual GenericCollection<T> Promote(IScaffoldCollection col)
        {
