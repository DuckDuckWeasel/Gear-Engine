...
namespace Scaffold
{
    ...
    {
        [Tooltip("Controls if the scaffold utilities are accessed from globals (e.g. say) or via a scaffold variable (e.g. scaffold.say). You can also choose to disable loading the scaffold module if it's not required by your script.")]
        [SerializeField] protected ScaffoldModuleOptions scaffoldModule = ScaffoldModuleOptions.UseGlobalVariables;
        ...
        {
            // Always register these ScaffoldLua utilities
            LuaEnvironment.RegisterType("Scaffold.PODTypeFactory");
            LuaEnvironment.RegisterType("Scaffold.ScaffoldPrefs");
        ...
        /// </summary>
        protected virtual void InitScaffoldModule()
        {
            if (scaffoldModule == ScaffoldModuleOptions.NoScaffoldModule)
            {
                return;
            }
            ...
            MoonSharp.Interpreter.DynValue value = interpreter.RequireModule("scaffold");
            if (value != null &&
                value.Type == DataType.Function)
            {
                scaffoldTable = value.Function.Call().Table;
            }
            if (scaffoldTable == null)
            {
                UnityEngine.Debug.LogError("Failed to create Scaffold table");
                return;
            }
            interpreter.Globals["scaffold"] = scaffoldTable;

            // Static classes
            scaffoldTable["time"] = UserData.CreateStatic(typeof(Time));
            scaffoldTable["playerprefs"] = UserData.CreateStatic(typeof(PlayerPrefs));
            scaffoldTable["prefs"] = UserData.CreateStatic(typeof(ScaffoldPrefs));
            ...
            if (testType != null)
            {
                ...
                scaffoldTable["test"] = UserData.CreateStatic(testType);
            }
            ...
            if (scaffoldModule == ScaffoldModuleOptions.UseGlobalVariables)
            {
                ...
                interpreter.Globals["scaffold"] = DynValue.Nil;

                // Note: We can't remove the scaffold table itself because of dependencies between functions
            }
        }
