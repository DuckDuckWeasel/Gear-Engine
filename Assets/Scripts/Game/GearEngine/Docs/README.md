...
* **`IGearSceneElement`**: Implemented by `GearBootstrap` (`Initialize`, `Enable`, `Disable`). Registered with the container as the scene-resident grid host.
* **`GearMechanicsInstaller`**: Plain C# installer constructed by `GearMechanicsScope`. Registers services, `GearBootstrap` as `IGearSceneElement`, `BoardConfigSO`, and `GearInventoryLoadoutSO`. Does **not** register views or view models.
* **`GearMechanicsScope`**: `LifetimeScope` with `[SerializeField]` references to **`NavigationSettings`**, **`navigationViewHolder`** (transform that parents the context `GearEngineView`), `BoardConfigSO`, `GearBootstrap`, and `GearInventoryLoadoutSO`. Installs **`NavigationInstaller`**, **`AddressablesInstaller`**, **`CrossLayerObjectResolver`**, and **`GearMechanicsInstaller`**; opens the screen via **`GearEngineNavigationEntry`** (`INavigation.Open(new GearEngineViewModel())`).
* **`GearBootstrap`**: On **`GearGrid_Root`**; owns a single `initialGear` for the centre cell and instantiates the grid visuals.
* **`GearInventoryLoadoutSO`**: ScriptableObject listing starting inventory gears. Place assets under `Assets/Game/GearEngine/Configs/` (generated by **GearEngine → Step 1** as `GearInventoryLoadout.asset`).
* **`GearEngineView`**: Parent `View<GearEngineViewModel>`; sub-widgets (`SimulationControlView`, `GearInventoryView`, `BoardView`) are `ViewComponent<T>` and are bound from `OnBind` (view is visual-only; the view model owns services and passes `IObjectResolver` to inventory slots only).
* **`GearEngineNavigationEntry`**: VContainer `IStartable` that opens the gear engine `GearEngineViewModel` through **`INavigation`** after the scope is built.
...
To merge with another mechanic in one scene, add a parent `LifetimeScope` (or child scope), duplicate the `GearEngine_Root` subtree, assign **`navigationSettings`**, **`navigationViewHolder`** (root that parents the canvas with `GearEngineView`), **`boardConfig`**, **`bootstrap`**, and **`loadout`**.
