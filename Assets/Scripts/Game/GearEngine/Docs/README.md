...
* **`GearMechanicsScope`**: `LifetimeScope` with `[SerializeField]` references to `BoardConfigSO`, `GearBootstrap`, `GearInventoryLoadoutSO`, and optional `GearEngineSceneBootstrap`.
...
* **`GearEngineView`**: Parent `View<GearEngineViewModel>`; sub-widgets (`SimulationControlView`, `GearInventoryView`, `BoardView`) are `ViewComponent<T>` and are bound from `OnBind`.
* **`GearEngineSceneBootstrap`**: For standalone scenes, creates a `GearEngineViewModel`, injects it, injects the view, and calls `Bind`. Navigation-driven setups open the same view from a view config instead.
...
To merge with another mechanic in one scene, add a parent `LifetimeScope` (or child scope), duplicate the `GearEngine_Root` subtree, assign the three scope references (`boardConfig`, `bootstrap`, `loadout`), and wire `GearEngineSceneBootstrap` to the `GearEngineView` on your canvas.
