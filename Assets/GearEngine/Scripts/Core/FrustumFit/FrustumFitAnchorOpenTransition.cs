...
namespace GearEngine.FrustumFit
{
    ...
    public static class FrustumFitAnchorOpenTransition
    {
        ...
        /// then restores <c>applyEveryFrame</c> when the tween finishes.
        /// </summary>
            ...
            bool restoreApplyEveryFrameAfter)
        {
            ...
            Action restore = shouldRestore ? () => RestoreApplyEveryFrame(anchors) : null;
        ...
        private static void RestoreApplyEveryFrame(IReadOnlyList<FrustumFitAnchor> anchors)
        {
            for (int i = 0; i < anchors.Count; i++)
            {
                FrustumFitAnchor anchor = anchors[i];
