...
namespace GearEngine.GearEngine.Visuals
{
    public class GearView : SerializedMonoBehaviour
    {
        ...
        [SerializeField] private IGridNode targetNode;
        [SerializeField] private Transform cachedVisual;
        [SerializeField] private BoardConfigSO boardConfig;
        [SerializeField] private GameObject chargeVisualObj;
        [SerializeField] private Transform chargeFillTransform;
        [SerializeField] private SpriteRenderer chargeFillRenderer;
        [SerializeField] private float currentVisualFill = 0f;
        [SerializeField] private static Sprite squareSpriteCenter;
        [SerializeField] private static Sprite squareSpriteBottom;
        ...
        {
            if (!TryGetChargeFlags(configData, out bool hasIcon, out bool hasCharge))
            {
                return;
            }

            InitSprites();
            CreateChargeVisual(configData, hasIcon);
            InitializeChargeFill(hasCharge);
        }

        private bool TryGetChargeFlags(GearConfigData configData, out bool hasIcon, out bool hasCharge)
        {
            hasIcon = configData != null && configData.UIIcon != null;
            hasCharge = configData != null && targetNode is BaseGearNode && configData.MaxCharge > 0;
            return hasIcon || hasCharge;
        }

        private void InitializeChargeFill(bool hasCharge)
        {
            if (!hasCharge)
            {
                return;
        ...
        private void CreateChargeVisual(GearConfigData configData, bool hasIcon)
        {
        ...
        private void ConfigureChargeMaterial()
        {
