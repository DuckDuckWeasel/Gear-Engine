...
* **`GearMechanicsScope`**: `LifetimeScope` with **`NavigationSettings`**, **`navigationViewHolder`** (transform that parents the context `GearEngineView`), **`BoardRulesSO`**, **`GearEngineFeatureToggleSO`**, and optional **`GearTestSceneBootstrap`**. Installs **`NavigationInstaller`** (with the default single-scope view-model injector), **`AddressablesInstaller`**, and **`GearMechanicsInstaller`**. The screen is opened from **`GearTestSceneBootstrap`** or from any host that calls **`INavigation.Open(new GearEngineViewModel())`**. Board presentation layout uses **`BoardLayoutSO`** on the board view (not in DI).
* **`GearEngineStartData`**: Serializable payload with **`BoardLayoutData`** (e.g. on **`RaceStartData.gearEngineData`**). The Gear test scene uses **`GearTestSceneBootstrap.boardSeed`** instead. Hydration of the board service runs through **`IBoardService.LoadLayout`**; **`GearEngineViewModel`** / **`BoardViewModel`** own presentation hydration. Tray contents come from **`IInventoryService.Owned`** minus placed gears (**`GearInventoryViewModel`** rebuilds **`TrayItems`** from inventory + board events).
* **`GearTestSceneBootstrap`**: Scene `MonoBehaviour` implementing **`IInitializable`**; has serialized **`BoardLayoutData boardSeed`**, calls **`IBoardService.LoadLayout(boardSeed)`** when non-empty, then **`INavigation.Open(new GearEngineViewModel())`**.
