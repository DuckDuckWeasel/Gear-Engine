...
namespace GearEngine.CarSimulation.Tracks
{
    ...
    public class SplinePropGenerator : MonoBehaviour
    {
        ...
        public abstract class PropRule
        {
            ...
            [Tooltip("Minimum horizontal distance from every part of the track spline. 0 = disabled.")]
            public float minimumSplineClearance = 0f;
        }
        ...
        public void Generate(IReadOnlyList<PropRule> rules)
        {
            ClearProps();
            if (track == null || rules == null || rules.Count == 0)
            {
                return;
            }
            ...
            {
                foreach (PropRule rule in rules)
                {
                    if (rule == null)
                    {
                        continue;
                    ...
                    if (UnityEngine.Random.value > rule.chancePerSegment)
                    {
                        continue;
                    }
                    ...
                    if (isCurvesRule)
                    {
                        threshold = ((Curves)rule).curvatureThreshold;
                    }

                    if (isStraightsRule)
                    {
                        threshold = ((Straights)rule).curvatureThreshold;
                    }
                    ...
                    for (int i = 0; i < amount; i++)
                    {
                        ...
                        if (isCurvesRule || isStraightsRule)
                        {
                            ...
                            if (isCurvesRule && !isCurve)
                            {
                                continue;
                            }

                            if (isStraightsRule && isCurve)
                            {
                                continue;
                            }
                        }
                        ...
                        foreach (Placement placement in placements)
                        {
                            ...
                            if (IsWithinSplineClearance(finalPos, spline, curveCount, rule.minimumSplineClearance))
                            {
                                continue;
                            }
        ...
        private bool IsWithinSplineClearance(
            Vector3 position,
            Spline spline,
            int curveCount,
            float minimumClearance)
        {
            ...
            float horizontalScale = Mathf.Max(
                Mathf.Abs(track.transform.lossyScale.x),
                Mathf.Abs(track.transform.lossyScale.z));
            float clearance = minimumClearance * horizontalScale;
            float clearanceSquared = clearance * clearance;
            int sampleCount = Mathf.Max(64, curveCount * 32);

            for (int sampleIndex = 0; sampleIndex <= sampleCount; sampleIndex++)
            {
                float sampleT = sampleIndex / (float)sampleCount;
                Vector3 splinePosition = track.transform.TransformPoint(
                    SplineUtility.EvaluatePosition(spline, sampleT));
                splinePosition.y = position.y;
                if ((position - splinePosition).sqrMagnitude < clearanceSquared)
                {
                    return true;
                }
            }
        ...
        {
            foreach (GameObject obj in generatedProps)
            {
                if (obj != null)
                {
