...
namespace GearEngine.CarSimulation.PhysicsSimulation
{
    public class SplineCarRunnerService : ISimulationRunnerService, ITickable
    {
        ...
        private readonly PhysicsSimulationConfig config;
        private readonly List<SplineCarRunnerContext> activeRunners = new List<SplineCarRunnerContext>();

        public SplineCarRunnerService(PhysicsSimulationConfig config)
        {
            this.config = config;
        }
        ...
        {
            if (initParams is not PhysicsInitParams physics)
            {
                throw new InvalidOperationException(
                    $"[SplineCarRunnerService] Expected PhysicsInitParams but got {initParams?.GetType().Name}");
            }
            InitializeRun(physics.Controller, physics.Track, physics.Stats, physics.Entity);
        }

        /// <inheritdoc/>
        public void RemoveDriver(CarEntity entity)
        {
            activeRunners.RemoveAll(c => c.entity == entity);
        }
        ...
        private void SetupPrometeoTouchOverride(SplineCarRunnerContext ctx)
        {
            ...
            ctx.targetCar.throttleButton = ctx.aiThrottle.gameObject;
            ctx.targetCar.reverseButton = ctx.aiReverse.gameObject;
            ctx.targetCar.turnLeftButton = ctx.aiLeft.gameObject;
            ctx.targetCar.turnRightButton = ctx.aiRight.gameObject;
            ctx.targetCar.handbrakeButton = ctx.aiBrake.gameObject;
